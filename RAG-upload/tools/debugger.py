...
def validate_database_integrity(args):
    ...
    print(f"Checking Pinecone namespace: '{namespace or 'default'}'")
    ...
    if missing_files:
        print(
            f"\nFound {len(missing_files)} files that are in tracking JSON but missing from Pinecone:"
        )
        ...
        print("\n✓ All tracked files are present in Pinecone database")
    ...
    if args.check_untracked:
        print("\nChecking for untracked Pinecone documents...")
        ...
        if untracked:
            ...
            if len(untracked) > 10:
                print(f" ... and {len(untracked) - 10} more documents")
        else:
            print("✓ All Pinecone documents are tracked in JSON")
